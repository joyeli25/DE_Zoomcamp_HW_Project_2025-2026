...
```sql
SELECT count(1)
FROM `terraform-project-485722.dbt_jli.fct_monthly_zone_revenue`
```
...
and cast(EXTRACT(YEAR FROM revenue_month) as string)="2020"
...
```sql
SELECT service_type, 
cast(EXTRACT(YEAR FROM revenue_month) as string),
cast(EXTRACT(MONTH FROM revenue_month) as string),
sum(total_monthly_trips)
FROM `terraform-project-485722.dbt_jli.fct_monthly_zone_revenue`
where service_type="Green"
and cast(EXTRACT(YEAR FROM revenue_month) as string)="2019"
and cast(EXTRACT(MONTH FROM revenue_month) as string)="10"
...
> (https://d37ci6vzurychx.cloudfront.net/trip-data/fhv_tripdata_2019-01.parquet) --43,244,696 rows

> (https://github.com/DataTalksClub/nyc-tlc-data/releases/download/fhv/fhv_tripdata_2019-01.csv.gz). --43,261,276 rows
...
```sql
SELECT count(*) FROM `terraform-project-485722.dbt_jli.stg_fhv_tripdata_csv` 
```
---
